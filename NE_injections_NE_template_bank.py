...
#coutour plot
plt.figure()
cp = plt.contourf(masses1,masses2,global_matches, cmap="plasma")
plt.colorbar()
plt.xlabel('mass 1 (solar mass)')
plt.ylabel('mass 2 (solar mass)')
plt.title('Fitting factor contour plot for 10*10 non-eccentric injections with the non-eccentric template bank')
plt.show()
end=datetime.now()
